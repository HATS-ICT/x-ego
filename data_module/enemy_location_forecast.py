...
try:
    from ..dataset.enemy_location_forecast import EnemyLocationForecastDataset, enemy_location_forecast_collate_fn
except ImportError:
    from dataset.enemy_location_forecast import EnemyLocationForecastDataset, enemy_location_forecast_collate_fn


class EnemyLocationForecastDataModule(BaseDataModule):
    """
    Lightning DataModule for multi-agent enemy future location prediction dataset.
    
    Provides train/validation/test splits and data loading functionality 
    for multi-agent enemy future location prediction tasks.
    """
    
    def __init__(self, cfg: Dict[str, Any]):
        """
        Initialize Enemy Future Location Prediction DataModule.
        ...
        # Multi-agent enemy future location prediction parameters
    ...
    def _create_base_dataset(self):
        """Create the base enemy future location prediction dataset."""
        return EnemyLocationForecastDataset(cfg=self.cfg)
    
    def _get_collate_fn(self):
        """Get the collate function for enemy future location prediction."""
        return enemy_location_forecast_collate_fn
