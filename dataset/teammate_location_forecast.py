...
try:
    from utils.dataset_utils import get_team_transcription_path_from_video_path, extract_text_segments_from_transcription
except ImportError:
    import sys
    from pathlib import Path
...
def teammate_location_forecast_collate_fn(batch):
    ...
    for key in batch[0].keys():
        ...
        if key in ['team_side', 'agent_ids', 'raw_text']:
            ...
            # For tensors (video, future_locations, team_side_encoded, text, text_attention_mask), use default collate
...
class TeammateLocationForecastDataset(BaseVideoDataset, Dataset):
    ...
    def _get_text_clip_from_recording(self, video_path: str, start_seconds: float, end_seconds: float) -> List[str]:
        ...
        if not Path(transcription_path).exists():
            return [""]
        transcription_text = extract_text_segments_from_transcription(transcription_path, start_seconds, end_seconds, allow_overflow=True)
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.with_communication:
            ...
            representative_video_path_full = Path(self.path_config['data']).parent / representative_video_path
