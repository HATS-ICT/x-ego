...
class RoundOutcomeReasonCreator(TaskCreatorBase):
    """
    ...
    def _extract_segments_from_round(self, match_id: str, round_num: int,
                                     config: Dict[str, Any]) -> List[Dict]:
        """Extract segments for round outcome reason prediction."""
        segment_length_sec = config['segment_length_sec']
        ...
        reason_label = reason_mapping[reason]
    ...
    def _create_output_csv(self, all_segments: List[Dict], config: Dict[str, Any]) -> pd.DataFrame:
        """Create output CSV for round outcome reason prediction."""
        output_rows = []
        idx = 0
