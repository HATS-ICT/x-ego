...
EXP_PREFIX = "main_contra_with_accu"
...
def get_training_settings(model: str) -> tuple[int, int]:
    try:
        return TRAINING_SETTINGS[model]
    except KeyError as exc:
        raise ValueError(f"No contrastive training settings configured for model: {model}") from exc
...
def main():
  ...
  meta.run_name={run_name}"""
    ...
    for model in MODELS:
        batch_size, accumulate_batches = get_training_settings(model)
        print(
            f"    {model}: batch_size={batch_size}, "
            f"contrastive_accumulate_batches={accumulate_batches}, "
