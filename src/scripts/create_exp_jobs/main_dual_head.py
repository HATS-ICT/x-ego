...
# Settings to sweep: (random_mask_enable, reconstruction_enable, short_name)
SETTINGS = [
    (False, True, "nomask-recon"),   # setting 1: no random mask, with reconstruction
    (True, True, "mask-recon"),      # setting 2: with random mask, with reconstruction
]
...
uv run python main.py --mode train --task contrastive \\
  ...
  model.reconstruction.enable={reconstruction_enable} \\
...
def main():
    ...
    for model in MODELS:
        ...
        for random_mask_enable, reconstruction_enable, setting_name in SETTINGS:
            ...
            reconstruction_str = str(reconstruction_enable).lower()
            ...
            body = JOB_BODY.format(
                ...
                reconstruction_enable=reconstruction_str,
  ...
  model.reconstruction.enable={reconstruction_str} \\
  data.batch_size={batch_size} \\
  data.num_workers=8 \\
