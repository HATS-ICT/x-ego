...
def validate_config(config):
    ...
    for section, keys in optional_sections.items():
        if section in config:
            for key in keys:
                if key not in config[section]:
                    raise ValueError(f"Missing required config key in optional section: {section}.{key}")
    ...
    for section_path, keys in nested_required_keys.items():
        ...
        try:
            ...
            raise ValueError(f"Missing required nested config section: {section_path}")
        
        # Check required keys in the nested section
        for key in keys:
            if key not in current:
                raise ValueError(f"Missing required config key: {section_path}.{key}")
    
    # Validate contrastive style is supported
    if 'training' in config and 'contrastive' in config['training']:
        contrastive_style = config['training']['contrastive'].get('style')
...
def apply_config_overrides(config, overrides):
    """Apply config overrides using OmegaConf
    
    Args:
        config (DictConfig): Original OmegaConf configuration
    ...
    print(f"Applying {len(overrides)} config overrides:")
    
    for override_str in overrides:
        try:
            # OmegaConf can parse the override string directly
            override_config = OmegaConf.from_dotlist([override_str])
            
            # Show what's being changed
            key_path = override_str.split('=')[0]
            old_value = OmegaConf.select(config, key_path, default='<not set>')
            new_value = override_str.split('=')[1]
            print(f"  {key_path}: {old_value} -> {new_value}")
            
            # Merge the override into the main config
